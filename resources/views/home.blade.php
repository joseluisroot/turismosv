...
<body>
    ...
    <main>
        ...
        @if($sponsoredCampaign)<section class="sponsored-section"><div><span>Contenido patrocinado</span><small>Publicidad · No modifica rankings ni calificaciones</small></div><h2>{{ $sponsoredCampaign->title }}</h2><p>{{ $sponsoredCampaign->description }}</p><footer><b>Patrocinado por {{ $sponsoredCampaign->sponsor_name }}</b><a href="{{ route('sponsorships.click',$sponsoredCampaign) }}" rel="sponsored nofollow">Conocer propuesta →</a></footer></section>@endif
