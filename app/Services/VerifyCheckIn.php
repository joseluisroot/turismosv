...
class VerifyCheckIn {
    public function __construct(private readonly AwardPassportProgress $awardPassportProgress) {}
    public function handle(CheckIn $checkIn,string $method,?int $qrCodeId=null,?int $verifiedBy=null,?string $note=null): PassportStamp {
        return DB::transaction(function() use($checkIn,$method,$qrCodeId,$verifiedBy,$note){
            ...
            $stamp=PassportStamp::query()->firstOrCreate(['check_in_id'=>$checkIn->id],['public_id'=>(string)Str::uuid(),'user_id'=>$checkIn->user_id,'place_id'=>$checkIn->place_id,'stamp_code'=>'SV-'.strtoupper(Str::random(10)),'earned_at'=>$checkIn->verified_at??now()]);
            $this->awardPassportProgress->handle($stamp);
            return $stamp;
        });
    }
}
